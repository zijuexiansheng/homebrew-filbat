class PythonAT2714 < Formula
  ...
  def install
    ...
    {
      "easy_install" => "easy_install-2.7.14",
      "pip" => "pip-2.7.14",
      "python" => "python-2.7.14",
      "python-config" => "python-2.7.14-config",
    ...
    <<~EOS
      ...
      if os.path.realpath(sys.executable).startswith('#{rack}'):
          ...
          try:
              ...
              build_time_vars['LINKFORSHARED'] = '-u _PyMac_Error #{opt_prefix}/local/Frameworks/Python.framework/Versions/2.7/Python'
          except:
              pass  # remember: don't print here. Better to fail silently.

          # Set the sys.executable to use the opt_prefix
          sys.executable = '#{opt_bin}/python-2.7.14'
    EOS
  end
  ...
  def caveats; <<~EOS
    ...
    Pip and setuptools have been installed. To update them
      pip-2.7.14 install --upgrade pip setuptools

    You can install Python packages with
      pip-2.7.14 install <package>
  ...
  test do
    ...
    system "#{bin}/python-2.7.14", "-c", "import sqlite3"
    # Check if some other modules import. Then the linked libs are working.
    system "#{bin}/python-2.7.14", "-c", "import Tkinter; root = Tkinter.Tk()" if OS.mac?
    system "#{bin}/python-2.7.14", "-c", "import gdbm"
    system bin/"pip-2.7.14", "list"
  end
end
